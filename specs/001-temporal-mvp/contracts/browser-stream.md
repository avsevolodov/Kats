...
One subscription per socket in MVP. Client can issue Subscribe, Cancel, Ping. Start через REST. Это прикладной duplex канал, а не SSE; сервер передаёт события, клиент — управление.
...
Разрешённые event kinds: RunAccepted, RunStatusChanged, OperationStarted, OutputBatch, OutputTruncated, CancelRequested, OperationCompleted, RunCompleted. Текст всегда рендерится escaped; Markdown/HTML агента не исполняется.
